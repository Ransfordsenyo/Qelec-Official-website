...
function processPayment(amount) {
    ...
    const paystackPublicKey = 'https://paystack.shop/pay/wxn2ue3298'; // Your Paystack public key
...
// Update your frontend script.js with these functions:

const API_BASE_URL = process.env.NODE_ENV === 'production' 
    ? 'https://your-vercel-app.vercel.app/api' 
    : 'http://localhost:5000/api';

async function initializePayment(amount, email, name, sponsorshipLevel = 'custom', message = '') {
    try {
...
// Update your payment button click handler
paystackButton.addEventListener('click', async () => {
    ...
    // Collect donor information (you might want to add form fields for this)
    const donorEmail = prompt('Please enter your email address:');
    const donorName = prompt('Please enter your name:');
    
    if (!donorEmail || !donorName) {
        showNotification('Email and name are required for sponsorship', 'error');
        return;
    }
    console.log("Qelec Website Loaded Successfully");

    await initializePayment(selectedAmount, donorEmail, donorName);
});
